	...
	<view>
		...
		<view class="layer-body">
			...
			<view class="layer-list">
				<view class="layer-container" id="slideBg" :style="{transform:`translateX(${slide}rpx)`}">
					<view v-for="(monthData, index) in date" :key="index" class="month">
						<view class="month-title" :class="'m-' + monthData[0].year + '-' + monthData[0].month" :style="'z-index:' + index">
							<uni-icon :type="leftIcon" :color="color" size="24" @tap="onClickLeft"></uni-icon>
							<view style="margin:0 50upx">{{ monthData[0].year + '年' + monthData[0].month + '月' }}</view>
							<uni-icon :type="rightIcon" :color="color" size="24" @tap="onClickRight"></uni-icon>
						</view>
						<view class="week-box">
							<text class="week-box-item" v-for="(item, tmpIndex) in weekNameArr" :key="tmpIndex">{{ item }}</text>
						</view>
						<view class="month-content" v-if="isFlag==0||isFlag==1">
							<view v-for="(data, index2) in monthData" :key="index2" class="day" :data-index="index" :data-indexs="index2"
							 :class="
										data.re < today
											...
											? ' begin' : data.date == choiceDate[1].date 
											? ' end' : '')
											: data.act.subject==''?'disabled'
											: data.act.subject==-1?'disabled'
											: data.act.subject==-2?'disabled'
											:''
									"
							 :style="index2 == 0 ? 'margin-left:' + weeks[index] * (100 / 7) + '%' : ''" @tap="dayClick">
								<view class="day-content">
									...
									<text class="day-tip" v-if="data.act.subject!=-2">
										{{ data.act.subject==-1?'无房':data.act.subject==-2?'':data.act.subject==-3?'':data.act.subject!=''?'￥'+data.act.subject:'' }}
									</text>
									<image v-if="data.act.subject==-2" style="width:20upx;height:20upx;margin-top:6upx;" :src="urlImage+'lock1.png'"></image>
								</view>
								<view class="beginTip" v-if="choice === false">请选择离店日期</view>
								<view class="endTip" v-if="choice">{{ dayCount2 }}</view>
							</view>
						</view>
						<view class="month-content" v-if="isFlag==2">
							<view v-for="(data, index2) in monthData" :key="index2" class="day" :data-index="index" :data-indexs="index2"
							 :class="
										data.re < today
											? 'disabled'
											...
											? 'date-active' 
											+ (data.date == choiceDate[0].date 
											? ' date-begin' : data.date == choiceDate[1].date 
											? ' date-end' : '')
											: data.act.subject==''?'disabled'
											: data.act.subject==-1?'active'
											:''
									"
							 :style="index2 == 0 ? 'margin-left:' + weeks[index] * (100 / 7) + '%' : ''" @tap="dayClick">
								<view class="day-content">
...
<script>
	...
	export default {
		...
		created() {
			// this.$store.commit('changeIsShowPage',true)
		...
		methods: {
			...
			getHouseCalendar() {
				...
				uni.request({
					url: this.getApp.api.index.houseCalendar,
					method: 'GET',
					data: {
					...
					success: res => {
						if (res.data.code == 0) {
							this.price = res.data.calendarDate
							this.init();
							this.showCalendar()
							// console.log(JSON.stringify(this.price))
							// console.log('拿到数据后' + this.price.length)
						}
					}
				});
			},
					...
					for (let j = 0; j < mList.length; j++) {
						...
						for (let k = 0; k < t_days[j]; k++) {
							...
							if (dayscNow < daysCount) {
								...
								var subject = ''
								// console.log('循环前' + this.price.length)
								if (dayscNow - 1 < this.price.length) {
									if (this.price[dayscNow - 1].order_status == 0) {
										if (this.price[dayscNow - 1].house_close == 1) {
											if (this.price[dayscNow - 1].float_price != null) {
												subject = this.price[dayscNow - 1].float_price
											} else {
												subject = this.price[dayscNow - 1].base_price
											}
										} else {
											subject = -2
										}
			...
			selectday: function(index, indexs, isUserClick) {
				...
				if (curDate.act.subject == '') {
					//如果是用户点击没有价格的日期的话，就返回 
					if (isUserClick) return;
				}
				if (curDate.act.subject == -1) {
					//如果是用户点击无房的日期的话，就返回 
					if (isUserClick) return;
				}
				if(this.isFlag!=2){
					if (curDate.act.subject == -2) {
						//如果是用户点击锁房的日期的话，就返回 
						if (isUserClick) return;
					}
				}
				if(this.isFlag==2){
					...
					todayDate[0] = curDate;
					todayDate[1] = curDate;
					this.$emit("pickData", todayDate)
				}
				// console.log(curDate.re)
				curDate.selected = 1;
				curDate.act.tip = '入住';
				// console.log(this.dateFlag.index)
				if (this.dateFlag.date && curDate.dateTime < this.dateFlag.date.dateTime) {
					...
					// console.log(this.date[flagIndex][flagIndexs].selected)
					if(this.isFlag==2){
						for (let i in that.choiceDateArr) {
							if (that.choiceDateArr.length != 0) {
								if (that.choiceDateArr[i].act.subject == -1) {
									nonFlag = true;
									that.choiceDateArr = []
								}
							}
						...
						for (let i in that.choiceDateArr) {
							if (that.choiceDateArr.length != 0) {
								if (that.choiceDateArr[i].act.subject == -1 || that.choiceDateArr[i].act.subject == -2) {
									nonFlag = true;
									that.choiceDateArr = []
								}
							}
					...
					// console.log(that.choiceDate)
					//设置开始和结果两个日期
					if (that.choiceDateArr.length != 0) {
						this.choiceDate[0] = that.choiceDateArr[0];
						this.choiceDate[1] = that.choiceDateArr[that.choiceDateArr.length - 1];
						var pickTime = [this.choiceDate[0], this.choiceDate[1]]
						this.$emit("pickData", pickTime,count+1)
					} else {
						var pickTime = []
						this.$emit("pickData", pickTime)
					}
					if (nonFlag) {
						var that = this;
						this.date.forEach(function(dataItems) {
...
* 
* */
	$themeColor: $uni-text-color-dateGreen;
	$dateColor:$uni-text-color-dateOrange;
