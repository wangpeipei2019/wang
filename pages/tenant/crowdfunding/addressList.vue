<template>
	<view v-if="isShow">
		<uni-nav-bar fixed="true" statusBar="true" left-icon="back" @click-left="comeBack()" @click-right="addAddress" title="添加收件地址" rightText="新增"></uni-nav-bar>
		<view class="uni-list">
			<checkbox-group @change="checkboxChange">
				<label class="uni-flex uni-list-item" v-for="item in inputAddress" :key="item.id">
					...
					<image class="del" @tap.stop="delModal(item.checked,item.id)" :src="imageUrl+'nav_icon_delete.png'" mode="widthFix"></image>
				</label>
			</checkbox-group>
		</view>
		<button type="primary" class="add-btn" @tap="addFriend">添加收件地址</button>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			addAddress() {
				uni.navigateTo({
					url: '/pages/tenant/crowdfunding/address'
				})
			},
			comeBack() {
				uni.navigateBack({})
