	...
	<view class="setting">
		...
		<view class="uni-flex uni-column listWrap">
			<view class="uni-flex uni-row settingList">
				...
				<view class="right">
					<input type="text" v-model="email" placeholder-style="color:#c1c1c1" placeholder="您的email地址" />
				</view>
			</view>
		</view>
		...
		methods: {
			...
			togglePopup(type) {
				...
				if (this.email==''||this.email==null) {
					...
					uni.request({
						url: this.getApp.api.crowdFunding.sendOutEmail,
						method: 'POST',
						data: {
			...
			togglePopupOpen(type){
				this.type = type
				uni.navigateTo({
					url:'/pages/tenant/crowdfunding/emailVerification?email='+this.email
				})
			}
