	...
	<view class="setting">
		...
		<view class="uni-flex uni-column listWrap">
			<view class="uni-flex uni-row settingList">
				...
				<view class="right">
					<input type="text" v-model="email" @input="emailFn" placeholder-style="color:#a8a8a8" placeholder="您的email地址" />
				</view>
			</view>
		</view>
		...
		methods: {
			...
			togglePopup(type) {
				...
				if (this.email==''||this.email==null) {
					...
					uni.request({
					//	url: "http://113.200.212.12/ymshapi/sysemail/sendOutEmail",
			...
			togglePopupOpen(type){
				this.type = type
				uni.redirectTo({
					url:'/pages/setting/landlord/emailVerification?email='+this.email
				})
			}
